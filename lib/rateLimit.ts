...
/* Escrita em /api/agent/* (POST de leads e de follow-ups, balde único): teto
   por token, não por IP — o agente sai de um IP só, então o limite é o da
   campanha inteira. 60/h = 1 por minuto: folga de várias vezes sobre um pico
   realista de anúncio local, e ainda assim um token vazado não passa de
   ~1.400 gravações/dia, volume que se limpa pelo filtro de origem. GET não
   tem limite. Raciocínio completo em docs/INTEGRACAO-WHATSAPP.md. */
